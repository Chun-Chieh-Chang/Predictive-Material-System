import openpyxl
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
from openpyxl.utils import get_column_letter
import sys

sys.stdout.reconfigure(encoding='utf-8')

file_path = r'c:\Users\USER\Downloads\Project\Predictive-Material-System\新增資料夾\原料\色粉清單明細.xlsx'
wb = openpyxl.load_workbook(file_path)
ws = wb['色粉清單明細']

print(f"Original total rows: {ws.max_row}")

# Read all data rows
raw_rows = []
for r in range(3, ws.max_row + 1):
    vals = [ws.cell(row=r, column=c).value for c in range(2, 6)] # col 2..5: 色粉料號, 成分, 製造商, 原料名
    if any(vals):
        raw_rows.append(vals)

# Deduplicate preserving original order
seen = set()
unique_rows = []
for r in raw_rows:
    t = tuple(r)
    if t not in seen:
        seen.add(t)
        unique_rows.append(r)

print(f"Deduplicated count: {len(unique_rows)} unique rows")

# Styles
NAVY_HEADER = "1F497D"
WHITE = "FFFFFF"
BORDER_GRAY = "D9D9D9"

font_title = Font(name="微軟正黑體", size=14, bold=True, color="1F497D")
font_header = Font(name="微軟正黑體", size=11, bold=True, color=WHITE)
font_data = Font(name="微軟正黑體", size=10, color="000000")

fill_header = PatternFill(start_color=NAVY_HEADER, end_color=NAVY_HEADER, fill_type="solid")
fill_zebra = PatternFill(start_color="F8FAFC", end_color="F8FAFC", fill_type="solid")
fill_white = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid")

thin_side = Side(border_style="thin", color=BORDER_GRAY)
border_cell = Border(left=thin_side, right=thin_side, top=thin_side, bottom=thin_side)

align_center = Alignment(horizontal="center", vertical="center", wrap_text=True)
align_left = Alignment(horizontal="left", vertical="center", wrap_text=True)

# Recreate sheet cleanly
wb.remove(ws)
ws_new = wb.create_sheet(title="色粉清單明細", index=0)

# Title (Row 1)
ws_new.merge_cells("A1:E1")
ws_new["A1"] = "色粉原料清單"
ws_new["A1"].font = font_title
ws_new["A1"].alignment = Alignment(horizontal="left", vertical="center")
ws_new.row_dimensions[1].height = 28

# Header (Row 2)
headers = ['項次', '色粉料號', '成分', '製造商', '原料名']
ws_new.row_dimensions[2].height = 24
for col_idx, h in enumerate(headers, start=1):
    cell = ws_new.cell(row=2, column=col_idx, value=h)
    cell.font = font_header
    cell.fill = fill_header
    cell.alignment = align_center
    cell.border = border_cell

# Write unique data rows (Rows 3..38)
for r_idx, row_vals in enumerate(unique_rows):
    current_row = 3 + r_idx
    ws_new.row_dimensions[current_row].height = 22
    
    # Seq no (Col 1)
    seq_cell = ws_new.cell(row=current_row, column=1, value=r_idx + 1)
    seq_cell.font = font_data
    seq_cell.alignment = align_center
    seq_cell.border = border_cell
    
    for c_idx, val in enumerate(row_vals, start=2):
        cell = ws_new.cell(row=current_row, column=c_idx, value=val)
        cell.font = font_data
        cell.border = border_cell
        
        if c_idx == 2:  # 色粉料號
            cell.alignment = align_center
        else:  # 成分, 製造商, 原料名
            cell.alignment = align_left

    # Zebra fill for whole row
    for c_i in range(1, 6):
        if r_idx % 2 == 1:
            ws_new.cell(row=current_row, column=c_i).fill = fill_zebra
        else:
            ws_new.cell(row=current_row, column=c_i).fill = fill_white

# Column widths
ws_new.column_dimensions['A'].width = 10  # 項次
ws_new.column_dimensions['B'].width = 24  # 色粉料號
ws_new.column_dimensions['C'].width = 28  # 成分
ws_new.column_dimensions['D'].width = 36  # 製造商
ws_new.column_dimensions['E'].width = 38  # 原料名

wb.save(file_path)
print(f"Successfully saved deduplicated file to: {file_path}")
print(f"New sheet max_row: {ws_new.max_row}, max_col: {ws_new.max_column}")
