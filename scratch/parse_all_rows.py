...
# Category bounds (from visual grid)
cat_bounds = [
    (47.3, 110.1, "ABS (A)"),
    (110.1, 283.4, "PVC (B)"),
    (283.4, 323.7, "PC (C)"),
...
# Inspect rows 0..44 (total 45 material rows)
material_rows = []
for i in range(len(merged_y)-2): # exclude the bottom row y=503.3..510
    yt = merged_y[i]
    yb = merged_y[i+1]
    ...
    row_cells = {"原料種類": cat, "廠商": mfg}
    row_links = {}
    
    for c in col_defs[2:]:
        cname = c['name']
        col_full_name = f"{c['name']}\n{c['sub']}" if c['sub'] != c['name'] else c['name']
        ...
        # sort spans by y (line), then x
        # Group spans by line (y difference < 2.5)
        lines_in_cell = []
...
print(f"Parsed {len(material_rows)} material rows successfully.")

# Let's inspect model names and special cells
for r in material_rows:
    print(f"Row {r['row_idx']+1:2d} | Cat: {r['cells']['原料種類']:<12} | Mfg: {r['cells']['廠商']:<25} | Model: {r['cells']['型號']:<35} | Color: {r['cells']['顏色']}")
