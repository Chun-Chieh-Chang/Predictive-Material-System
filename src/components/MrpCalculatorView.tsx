  ...
  return (
    <>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2">
              <span className="bg-blue-950 text-blue-400 border border-blue-800/60 text-sm font-mono px-2 py-0.5 rounded-md">
                3-STAGE MRP ENGINE
              </span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm text-slate-400 font-semibold">切換品號:</span>
            ...
            {onNavigateToSettings && (
              <button
                ...
                className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-sm font-medium bg-slate-950 hover:bg-slate-800 text-purple-300 border border-purple-800/60 transition-colors ml-2"
        ...
        {params && (
          <div className="mt-4 pt-3 border-t border-slate-800/60 flex flex-wrap items-center gap-2 text-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-800/80 text-sm">
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <span className="text-sm text-slate-400 uppercase block font-medium">產品名稱 / 客戶</span>
            <span className="font-bold text-white mt-1 block text-sm">{result.productName} ({result.customerId})</span>
          </div>
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <span className="text-sm text-slate-400 uppercase block font-medium">需求版本 / 交期</span>
            <span className="font-mono font-bold text-cyan-400 mt-1 block text-sm">{result.versionNo} ({result.targetDate})</span>
          </div>
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <span className="text-sm text-slate-400 uppercase block font-medium">指定塑料 (RM SKU)</span>
            <span className="font-mono font-bold text-purple-400 mt-1 block text-sm">{result.rmSku}</span>
          </div>
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <span className="text-sm text-slate-400 uppercase block font-medium">最晚採購下單日</span>
              ...
              return (
                <div key={idx} className={`p-4 rounded-xl border ${borderClass} flex flex-col md:flex-row md:items-center justify-between gap-3 text-sm`}>
                  <div>
                    ...
                    <p className="mt-1.5 text-slate-200 leading-relaxed text-sm">
                      {alert.description}
                    </p>
                  </div>
                  <div className="shrink-0 bg-slate-950/70 px-3.5 py-2 rounded-lg border border-slate-800 text-sm font-sans font-medium text-slate-200">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 my-6 items-center">
          ...
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-blue-400 font-semibold block uppercase">A. 總需求量 Demand</span>
          ...
          {/* Step 2: FG Ready */}
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-emerald-400 font-semibold block uppercase">B. 成品在庫良品 FG Ready</span>
          ...
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-purple-400 font-semibold block uppercase">C. 待驗品有效良品 WIP Net</span>
        ...
        <div className="bg-blue-950/40 border border-blue-500/30 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <span className="text-sm text-blue-200 block">第 1 階計算結果：成品淨需求量 (FG Net Requirement)</span>
              <span className="text-lg font-bold text-white">
                需射出製造 <span className="text-cyan-300 font-mono">{result.fgNetRequirementQty.toLocaleString()}</span> PCS
              </span>
            </div>
          </div>
          <span className="text-sm font-mono bg-blue-900/60 text-blue-200 px-3 py-1.5 rounded-lg border border-blue-700/50">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20">
        ...
        <div className="mt-4">
          <span className="text-sm font-semibold text-slate-400 block mb-2">可適合成型模具清單 (M:N 多模具關聯支援)：</span>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {bom.is_primary_mold ? (
                      <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-sm font-bold px-2.5 py-0.5 rounded-full">
                        ★ 主模 (Primary)
                      </span>
                    ) : (
                      <span className="bg-slate-800 text-slate-300 text-sm px-2.5 py-0.5 rounded-full">
                        備用模 (Secondary)
                      </span>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3 text-sm">
                    <div>
                      <span className="text-sm text-slate-400 block">妥善/設計穴數</span>
                    ...
                    <div>
                      <span className="text-sm text-slate-400 block">成型週期</span>
                      <span className="font-mono text-slate-200">{moldInfo?.cycle_time_sec} 秒</span>
                    </div>
                    <div>
                      <span className="text-sm text-slate-400 block">日產能 (PCS/日)</span>
                  ...
                  {isDegraded && (
                    <div className="mt-2.5 text-sm text-amber-400 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 text-sm">
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-slate-400 uppercase block font-medium">整模克重 (Total Shot Weight)</span>
            <div className="font-mono font-bold text-base text-white mt-1">
              {result.totalShotWeightG} <span className="text-sm font-normal text-slate-400">g</span>
            </div>
          ...
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-slate-400 uppercase block font-medium">計算分母：妥善穴數 (Active)</span>
            <div className="font-mono font-bold text-base text-purple-400 mt-1">
              {result.activeCavities} <span className="text-sm font-normal text-slate-400">穴</span>
            </div>
            <span className="text-sm text-slate-400 mt-1 block">
          ...
          <div className="bg-purple-950/30 p-4 rounded-xl border border-purple-500/40">
            <span className="text-sm text-purple-300 uppercase block font-semibold">單穴耗用原料克重 (Unit Weight)</span>
            <div className="font-mono font-bold text-lg text-purple-200 mt-1">
              {result.unitWeightG.toFixed(3)} <span className="text-sm font-normal">g/穴</span>
            </div>
            <span className="text-sm text-purple-300/80 mt-1 block">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl shadow-black/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 my-6">
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-slate-400 uppercase font-semibold block">A. 毛需求 (含製程損耗)</span>
            <div className="text-xl font-mono font-bold text-white mt-1">
              {result.rmGrossRequirementKg.toLocaleString()} <span className="text-sm font-normal text-slate-400">KG</span>
            </div>
            <span className="text-sm text-slate-400 mt-1 block">
          ...
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-slate-400 uppercase font-semibold block">B. 原料可用庫存 (在庫+在途)</span>
            <div className="text-xl font-mono font-bold text-emerald-400 mt-1">
              {(result.rmOnHandKg + result.rmInTransitKg).toLocaleString()} <span className="text-sm font-normal text-slate-400">KG</span>
            </div>
            <span className="text-sm text-slate-400 mt-1 block">
          ...
          <div className="bg-slate-950/70 p-4 rounded-xl border border-slate-800">
            <span className="text-sm text-slate-400 uppercase font-semibold block">C. 原料淨缺口 (含安全庫存)</span>
            <div className="text-xl font-mono font-bold text-purple-400 mt-1">
              {result.rmNetRequirementKg.toLocaleString()} <span className="text-sm font-normal text-slate-400">KG</span>
            </div>
            <span className="text-sm text-slate-400 mt-1 block">
          ...
          <div className="bg-blue-950/40 p-4 rounded-xl border border-blue-500/40">
            <span className="text-sm text-blue-300 uppercase font-bold block">D. 建議採購下單量 (取整 MOQ)</span>
            <div className="text-xl font-mono font-bold text-blue-400 mt-1">
              {result.suggestedOrderQtyKg.toLocaleString()} <span className="text-sm font-normal text-slate-400">KG</span>
            </div>
            <span className="text-sm text-blue-300/80 mt-1 block">
        ...
        <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3">
            <div className="text-right">
              <span className="text-sm text-slate-400 uppercase block font-medium">建議最晚下單發 PO 日期</span>
              <span className="text-lg font-mono font-bold text-red-400">{result.suggestedOrderDate}</span>
            </div>
            <div className="h-10 w-[1px] bg-slate-800"></div>
            <div className="text-right">
              <span className="text-sm text-slate-400 uppercase block font-medium">下單倒數計時</span>
