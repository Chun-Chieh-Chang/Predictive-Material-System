...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
      <button
        ...
        className={[
          ...
          'group relative',
          isActive
            ? 'bg-sky-950/50 text-sky-300 font-semibold'
      ...
      >
        ...
        {isActive && (
          <span
            className="absolute left-0 top-1.5 bottom-1.5 w-0.5 bg-sky-500 rounded-full"
            style={{ boxShadow: '0 0 6px 1px rgba(14,165,233,0.5)' }}
          />
        )}
        ...
        <div className="flex-1 min-w-0">
          <div className={['text-sm truncate', isActive ? 'text-sky-200' : ''].join(' ')}>
            {item.label}
          </div>
    ...
    return (
      <div key={group.title} className="mb-1">
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-600 hover:text-slate-400 transition-colors cursor-pointer rounded-lg"
  ...
  const mobileHeader = (
    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
      <div className="flex items-center gap-2.5">
        <div className="w-8 h-8 bg-sky-600 rounded-lg flex items-center justify-center font-bold text-white text-sm shadow-md shadow-sky-600/30">
          料
        </div>
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="hidden lg:flex items-center gap-2.5 px-4 py-3.5 border-b border-slate-800/60 shrink-0">
        <div className="w-8 h-8 bg-sky-600 rounded-lg flex items-center justify-center font-bold text-white text-sm shadow-md shadow-sky-600/30 shrink-0">
          料
        </div>
            ...
            <div className="space-y-0.5 pb-2">
              ...
              >
                <ShieldCheck
                  className={[
                    'w-4 h-4 shrink-0 transition-colors duration-150',
                    activeTab === 'backup_settings' ? 'text-emerald-400' : 'text-slate-500 group-hover:text-slate-300',
                  ].join(' ')}
                />
                <div className="flex-1 min-w-0">
                  <div className="text-sm truncate">自動化備份</div>
                  <div className="text-[10px] text-slate-600 font-mono">Backup System</div>
                </div>
                {backupEnabled && (
  ...
  const desktopSidebar = (
    <aside
      className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 bg-slate-950 border-r border-slate-800/80"
      aria-label="桌面端導航側邊欄"
    >
      {sidebarContent}
    </aside>
