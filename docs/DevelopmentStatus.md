...
| `2c323dd` | fix(spec): 流程圖節點文字依內容行數動態上下置中 |
| `3b73998` | refactor(spec): 圖B六環節檢核晶片改兩兩並排（2+2+1）緊湊排列 |
| `a30b757` | fix(spec): 修正 routeEdge 未宣告 BW 導致渲染崩潰＋新增執行期煙霧測試 |
| `b2d01f4` | refactor(spec): 圖B六環節檢核改群組容器畫法（虛線子程序邊界、一進一出） |
| `0ca3776` | fix(spec): 六環節檢核邏輯修正 — 平行檢核非互斥、③/③′合併、移除重複齊套決策 |
| `971218b` | fix(ux): 章節展開/收合按鈕移至獨立頁浮動工具列（postMessage 控制） |
| `22c207c` | fix(spec): 流程圖箭頭全面對齊邊線中點（程式化審計通過） |
| `71667f5` | docs(spec): 入門導覽新增兩大主軸簡易流程與數據邏輯（一分鐘版） |
| `f0783a9` | fix(resilience): 修復 sandbox 受限環境未捕捉 SecurityError 崩潰（V-20260826-42） |

---

## 六、2026-08-26 進度摘要（V-20260826-37 ～ 43）

| 主題 | 內容 |
|------|------|
| 數據邏輯規格書 v3.1 | 備料補貨／交期估算兩大主軸互動流程圖（ISO 5807 標準符號、六種邊線路由零穿越）＋五欄式參數定義表＋統一核心鏈路整合分析；入門導覽（餐廳比喻＋術語浮窗＋FAQ＋兩大主軸一分鐘簡易版）；19 章預設摺疊 |
| 獨立全版面頁 | 「數據邏輯規格書」改 `?page=spec` 獨立頁開啟（無導航干擾、100dvh 響應式），SSOT postMessage 即時同步全域參數與七大資料表筆數；章節展開/收合按鈕整合至浮動工具列 |
| 預估多版本管理 | MRP 版本下拉選擇器＋版本衝擊分析（現版 vs 前一版）；最新版判定改 `created_at`→`version_no` 降序；需求交叉比對同期別僅採計最新版（修復重複累加失真） |
| 流程圖精煉系列 | 箭頭全面對齊邊線中點（程式化審計）；六環節檢核邏輯修正（平行檢核非互斥、③/③′合併、移除重複決策）；群組容器畫法＋2+2+1 緊湊排列；節點文字動態置中；執行期渲染煙霧測試（攔截 BW 未宣告型崩潰） |
| 韌性修復 | sandbox 受限環境未捕捉 SecurityError 崩潰修復＋main.tsx 沙盒防護 |
| 技能沉澱 | `interactive-flowchart-spec` skill（使用者層級＋專案可攜副本），沉澱流程圖規格製作全規範 |
