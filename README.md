...
| 版本 | 日期 | 說明 |
|------|------|------|
| **V-20260826-41~43** | 2026-08-26 | **整體優化＋韌性修復＋流程圖精煉版**：死碼清除與文件全面同步；sandbox 受限環境 SecurityError 崩潰修復＋main.tsx 沙盒防護；流程圖箭頭全面對齊邊線中點（程式化審計）、六環節檢核改群組容器畫法（2+2+1 緊湊排列）、節點文字動態置中、執行期渲染煙霧測試；新增 interactive-flowchart-spec 技能檔沉澱全規範。 |
| **V-20260826-44** | 2026-08-26 | **介面自適應縮放、xlsx 動態載入、主檔拖動捲動、工作台整合版**：Navbar 五級縮放驅動根字號（全專案 px→rem 遷移）；人工輸入 50–300% 縮放＋▲▼ 箭頭 5% 每格（範圍 60–140%）；xlsx 改動態 import（首屏 −143 kB gzip，−33%）；主檔寬表長按 300ms 拖動捲動含慣性 +60 fps rAF；業務/生管工作台嵌入完整功能檢視與角色專屬主檔維護區，消除跨頁導航。 |
